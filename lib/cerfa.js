...
//
// POURQUOI cheminGabarit() ET gabaritPresent() SONT EXPORTÉS (19/08/2026)
//
// Vercel n'embarque dans une fonction que les fichiers qu'il a vus LIRE dans
// le graphe de dépendances de cette fonction-là. Le traçage suit les imports :
// un readFileSync repéré ici fait embarquer le PDF partout où ce module est
// importé — donc dans api/envoyer.js, qui en a besoin.
//
// api/sante.js, lui, ne l'importait pas : il se contentait d'un existsSync sur
// le même chemin. Un existsSync ne fait rien embarquer, et le témoin annonçait
// « ABSENT » sur une chaîne qui, elle, générait très bien. Une demi-heure a été
// perdue à croire la production cassée.
//
// D'où ces deux exports : le témoin passe désormais par le module qui lit
// réellement le fichier, ce qui le fait embarquer dans la fonction de santé et
// rend le diagnostic vrai. C'est la mécanique employée par CERTIF, dont
// api/sante.js importe gabaritPresent depuis lib/cerfa-cu.js.
//
// Contrepartie assumée : importer ce module dans api/sante.js y amène pdf-lib,
// donc un paquet plus gros et un démarrage à froid plus lent. Sur une route de
// diagnostic appelée à la main, c'est sans conséquence — et un témoin qui
// mentait coûtait plus cher.

import { PDFDocument } from 'pdf-lib';
import { readFileSync, existsSync } from 'fs';
import { join } from 'path';

export const FICHIER = '6815-em-sd_31.pdf';

/** Chemin du gabarit dans le paquet de la fonction. */
export const cheminGabarit = () => join(process.cwd(), 'data', FICHIER);

/** Le gabarit est-il embarqué ? À n'appeler que depuis un module qui le lit. */
...
function lireGabarit() {
  if (!gabarit) {
    // Le message nomme le fichier et le geste : sans lui, l'erreur remontait
    // en ENOENT brut depuis readFileSync, à charge pour le lecteur de deviner
    // quel fichier manquait.
    if (!gabaritPresent()) {
      throw new Error(`gabarit absent : déposer l'imprimé 6815-EM-SD dans data/${FICHIER}`);
    }
...
// Les images ne viennent plus d'ici : voir lib/sceau.js. Elles sont rangées
// chiffrées, et la phrase secrète du notaire est la clé qui les ouvre — pas un
// mot de passe qu'on compare. Ce module ne fait que les dessiner.
