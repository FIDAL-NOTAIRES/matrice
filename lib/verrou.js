...
export async function verifier(req) {
  ...
  if (entraConfigure && entete.startsWith('Bearer ')) {
    ...
    try {
      ...
      // Dire LAQUELLE des revendications a échoué. « ERR_JWT_CLAIM_VALIDATION_FAILED »
      // seul a coûté une demi-heure : jose sait quel claim et pourquoi, autant le
      // remonter. Aucune donnée sensible là-dedans — ni le jeton, ni sa signature.
      const detail = e.claim ? ` (revendication « ${e.claim} » : ${e.reason || 'inattendue'})` : '';
