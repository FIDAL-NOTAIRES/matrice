...
export async function resoudre(noms) {
  ...
  for (const nom of uniques) {
    ...
    if (exacts.length === 1) {
      ...
      sortie.set(nom, {
        motif: 'aucune commune de ce nom — commune fusionnée ou disparue, ou orthographe à revoir',
        candidats: (Array.isArray(reponse) ? reponse : []).slice(0, 3).map((c) => `${c.nom} (${c.code})`),
      });
    } else {
      sortie.set(nom, {
        motif: `${exacts.length} communes portent ce nom — précisez le code INSEE`,
        candidats: exacts.slice(0, 6).map((c) => `${c.nom} (${c.code})`),
      });
    }
  }
 ...
 *
 * @returns {Promise<{perimes:Array, controleImpossible:boolean}>}
 */
export async function verifierCodes(codes) {
  ...
  return { perimes, controleImpossible };
}
