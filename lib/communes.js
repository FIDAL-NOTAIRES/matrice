...
// Les communes ASSOCIÉES et DÉLÉGUÉES ne sont pas des communes disparues.
//
// LOMME (59355) a été absorbée par Lille en 2000, mais elle subsiste comme
// commune associée, avec son code INSEE propre — et le cadastre tient des
// communes cadastrales pour elles. Les parcelles de Lomme sont référencées
// sous Lomme, pas sous Lille : écrire LILLE sur le formulaire serait une perte
// de précision, pas une correction.
//
// Elles ne figurent pas dans /communes, d'où la seconde interrogation. Le
// routage n'en souffre pas : il se fait sur les deux premiers caractères du
// code, identiques à ceux de la commune de rattachement.
async function interroger(url) {
  const r = await fetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'MATRICE/1.0 (FIDAL Notaires)' },
  });
  if (r.status === 404) return { ok: true, liste: [] };
  if (!r.ok) return { ok: false, statut: r.status };
  const brut = await r.json();
  return { ok: true, liste: Array.isArray(brut) ? brut : [brut].filter(Boolean) };
}
...
export async function resoudre(noms) {
  ...
  for (const nom of uniques) {
    ...
    let reponse, associees = [];
    try {
      const a = await interroger(`${API}?nom=${encodeURIComponent(nom)}&fields=code,nom,departement&limit=20`);
      if (!a.ok) { sortie.set(nom, { motif: `annuaire des communes indisponible (${a.statut})` }); continue; }
      reponse = a.liste;

      // Second passage : les communes associées et déléguées, absentes du
      // premier annuaire. C'est là que se trouve LOMME.
      const b = await interroger(
        `${API_ASSOCIEES}?nom=${encodeURIComponent(nom)}&fields=code,nom,chefLieu,type,departement&limit=20`);
      if (b.ok) associees = b.liste;
    } catch (e) {
    ...
    const exacts = reponse.filter((c) => normaliser(c.nom) === cible);
    const exactesAssociees = associees.filter((c) => normaliser(c.nom) === cible);

    if (exacts.length === 1) {
      sortie.set(nom, { code: exacts[0].code, nom: exacts[0].nom });
    } else if (exacts.length === 0 && exactesAssociees.length === 1) {
      const c = exactesAssociees[0];
      sortie.set(nom, {
        code: c.code, nom: c.nom,
        note: `commune ${c.type === 'commune-deleguee' ? 'déléguée' : 'associée'}`
          + (c.chefLieu ? ` — rattachée à ${c.chefLieu}` : ''),
      });
    } else if (exacts.length === 0) {
      sortie.set(nom, {
        motif: 'aucune commune de ce nom — nom incomplet, ou commune supprimée sans rattachement',
        candidats: [...propositions(reponse, 6), ...propositions(associees, 4)],
      });
    } else {
      sortie.set(nom, {
...
export async function verifierCodes(codes) {
  ...
  for (const code of [...new Set(codes.filter(Boolean))]) {
    ...
    try {
      ...
      if (!r.ok && r.status !== 404) { controleImpossible = true; continue; }

      let c = r.status === 404 ? null : await r.json();

      // Absent de /communes ? Ce n'est pas encore un code périmé : il peut
      // désigner une commune associée ou déléguée, que le cadastre distingue.
      if (!c || !c.code) {
        const a = await interroger(`${API_ASSOCIEES}?code=${encodeURIComponent(code)}&fields=code,nom`);
        if (!a.ok) { controleImpossible = true; continue; }
        c = a.liste.find((x) => x.code === code) || null;
      }
