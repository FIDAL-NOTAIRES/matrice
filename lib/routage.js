...
import { grouperEnvois, compterFormulaires } from './envois.js';
 ...
 *
 * Exportée pour lib/voisinage.js : une règle de sécurité écrite à deux endroits
 * finit par diverger. Le 31/08/2026, la déduction par voisinage ignorait les
 * réserves et routait le Pas-de-Calais vers `pelh.620.boulogne-sur-mer@`, une
 * adresse pourtant réservée parce qu'un pôle d'évaluation des locaux
 * d'habitation n'instruit pas les extraits de matrice.
 */
export function sousReserve(courriel, corrections) {
 ...
 * @param {object} appris       communes apprises, indexées par code INSEE,
 *                              issues de matrice_routage_appris
 ...
 */
export function router(commune, referentiel = {}, corrections = {}, appris = {}) {
  ...
  // Garde-fou de version. Un code v4 nourri d'un fichier v1 routerait faux en
  ...
  // Commune apprise : le service a répondu, donc l'adresse est établie. Cette
  // voie court-circuite le référentiel — elle est la seule à pouvoir affirmer
  // `etatAdresse: 'confirmee'`.
  const su = appris[code];
  if (su && su.destinataire) {
    return {
      statut: 'a_envoyer',
      destinataire: su.destinataire,
      serviceNom: su.service_nom || null,
      telephone: su.telephone || null,
      motif: null,
      departement: dep,
      etatAdresse: 'confirmee',
      origineAdresse: 'réponse reçue du service',
      resolution: `appris le ${String(su.appris_le || '').slice(0, 10)}`,
    };
  }
 ...
 *
 * Le groupement est délégué à lib/envois.js : depuis la déduction par
 * voisinage, une commune peut viser plusieurs services, et cette logique doit
 * vivre à un seul endroit — ici, dans vue-dossier.js et dans envoyer.js.
 */
export function routerPortefeuille(communes, referentiel, corrections, appris = {}) {
  const lignes = communes.map((c) => ({ ...c, ...router(c, referentiel, corrections, appris) }));
  ...
  return {
    ...
    resume: {
      ...
      // Une commune adressée à deux services donne deux Cerfa : le compte des
      // formulaires ne coïncide plus avec celui des communes prêtes.
